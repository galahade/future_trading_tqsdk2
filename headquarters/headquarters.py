...
from tqsdk2 import TqApi, TqAuth, TqBacktest, TqKq, TqRohon
...
class AccountManager:
    ...
    def __init__(self, trade_config: TradeConfigInfo):
        ...
        rohon_config = trade_config.rohon_account
        _tq_acc = trade_config.tq_account
        self._acc_type = trade_config.account_type
        if self._acc_type == 1:
            self.trade_account = None
        elif self._acc_type == 2:
            self.trade_account = TqRohon(
                td_url=rohon_config.url,
                broker_id=rohon_config.broker_id,
                app_id=rohon_config.app_id,
                auth_code=rohon_config.auth_code,
                user_name=rohon_config.user_name,
                password=rohon_config.password,
            )
            self.logger.info(f"融航账号信息: 用户名-{rohon_config.user_name}")
        else:
        ...
        self.tq_auth = TqAuth(_tq_acc.user_name, _tq_acc.password)  # type: ignore
...
class TradeManager:
    ...
    def __init__(self, acc_manager: AccountManager):
        ...
        if is_backtest:
            ...
            self.tqApi = TqApi(
                account=acc_manager.trade_account,
            ...
            self.tqApi = TqApi(account=acc_manager.trade_account, auth=acc_manager.tq_auth)
